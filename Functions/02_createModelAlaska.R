...
newData <-read_csv("newData.csv") #1884
newData <- newData %>%
  select(-1,-2)
dim(newData) #1882
str(newData)
...
Data_1883 <- data[c(1,2:1883)] #28 1883
...
pls2 <- plsr(BSiPercent~., ncomp = 10, data=Data_1883, validation = "CV", segments = 5)

predict(pls2, newdata = newData, ncomp = 3)
